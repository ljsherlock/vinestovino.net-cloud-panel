...
/**
 * Plugin Name: Dynamic Pricing and Discount Rules for WooCommerce (Premium)
        ...
        function wcdrfc_fs()
        {
            ...
            if ( !isset( $wcdrfc_fs ) ) {
                ...
                $wcdrfc_fs = fs_dynamic_init( array(
                    ...
                    'first-path' => 'admin.php?page=wcdrfc-page-get-started&send-wizard-data=true',
                    'contact'    => false,
                    'support'    => false,
                ),
                    'is_live'          => true,
...
if ( !defined( 'WDPAD_VERSION_LABEL' ) ) {
    ...
    if ( wcdrfc_fs()->is__premium_only() ) {
        ...
        if ( wcdrfc_fs()->can_use_premium_code() ) {
            define( 'WDPAD_VERSION_LABEL', strtoupper( wcdrfc_fs()->get_plan_title() ) );
            define( 'WDPAD_PROMOTIONAL_SLUG', 'pro_woo_discount' );
        } else {
