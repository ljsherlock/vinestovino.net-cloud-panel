...
class Admin {
	...
	public function add_listing_actions( $order ) {
		...
		printf(
			...
			esc_attr( isset( $listing_actions['invoice']['url'] ) ? esc_attr( $listing_actions['invoice']['url']. '&user_id=' . $order->get_user_id() . '&statement' ) : '' ),
			esc_attr( 'statement-of-account' ),
			esc_attr( 'Statement of Account' ),
			esc_attr( '/wp-content/plugins/woocommerce-pdf-invoices-packing-slips/assets/images/invoice.svg' ),
		);

		printf(
			'<a href="%1$s" class="button tips wpo_wcpdf %2$s" target="_blank" alt="%3$s" data-tip="%3$s" style="background-image:url(%4$s);"></a>',
			esc_attr( isset( $listing_actions['invoice']['url'] ) ? esc_attr( $listing_actions['invoice']['url'] . '&tax-reciept' ) : '' ),
			esc_attr( 'tax-receipt' ),
			esc_attr( 'Tax Reciept' ),
			esc_attr( '/wp-content/plugins/woocommerce-pdf-invoices-packing-slips/assets/images/invoice.svg' ),
		);

		printf(
			'<a href="%1$s" class="button tips wpo_wcpdf %2$s" target="_blank" alt="%3$s" data-tip="%3$s" style="background-image:url(%4$s);"></a>',
			esc_attr( isset( $listing_actions['invoice']['url'] ) ? esc_attr( $listing_actions['invoice']['url'] . '&invoice-packing-slip' ) : '' ),
			esc_attr( 'invoice-packing-slip' ),
			esc_attr( 'Invoice & Packing Slip' ),
			esc_attr( '/wp-content/plugins/woocommerce-pdf-invoices-packing-slips/assets/images/invoice.svg' ),
		);
	}
	...
	public function pdf_actions_meta_box( $post_or_order_object ) {
		...
		<ul class="wpo_wcpdf-actions">
			...
			$url                   = isset( $meta_box_actions['invoice']['url'] ) ? esc_attr( $meta_box_actions['invoice']['url'] . '&tax-reciept' ) : '';
			$class                 = 'button invoice exists';
			$alt                   = 'PDF Tax Reciept';
			$title                 = 'PDF Tax Reciept';
			$exists                = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill-rule="evenodd" d="M9,20.42L2.79,14.21L5.62,11.38L9,14.77L18.88,4.88L21.71,7.71L9,20.42Z"></path></svg>';
			printf(
				'<li><a href="%1$s" class="button %2$s" target="_blank" alt="%3$s">%4$s%5$s</a></li>',
				$url,
